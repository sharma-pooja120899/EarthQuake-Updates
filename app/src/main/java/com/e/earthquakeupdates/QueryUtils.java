...
public class QueryUtils {
    ...
    public static ArrayList<EarthCustom> giveLists(){
        ArrayList<EarthCustom> list=new ArrayList<>();

        try {
            JSONObject jsonRootObject = new JSONObject(SAMPLE_JSON_RESPONSE);
            JSONArray array=jsonRootObject.getJSONArray("features");
            ...
            for(int i=0;i<size;i++){
                ...
                JSONObject properties=arrObj.getJSONObject("properties");
                ...
                String vv = new SimpleDateFormat("LLL dd, yyyy ", Locale.ENGLISH).format(df);
                String timeHour=new SimpleDateFormat("HH:mm a",Locale.ENGLISH).format(df);

                list.add(new EarthCustom(output,location,vv,timeHour,palace,url));
            }
